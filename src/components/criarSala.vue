<template>
  ...
  <div class="container border-4 backdrop-blur-sm bg-white/20 border-opacity-10 border-black rounded-lg mx-auto w-4/5 relative z-10 flex items-center pb-16 my-24 md:my-8">
    <div class="w-full flex flex-col items-center justify-between relative z-10">
      <p class="flex items-center mt-2 font-bold text-5xl text-center md:text-5xl text-black">
        Criar Sala
      </p>
      ...
      <div class="w-full flex flex-row justify-center space-x-3.5">
        <div class="flex flex-col max-w-lg items-center text-lg mt-6 text-black">
          <p>Nome da Sala</p>
          <input v-model="roomName" class="rounded border-black bg-amber-100 border-2 text-gray-700 w-96 mt-2" placeholder="Nome da Sala" type="text" required>
        </div>
      </div>
      ...
      <div class="w-full flex flex-row justify-center space-x-3.5">
        <div class="flex flex-col max-w-lg items-start text-lg mt-6 text-black">
          ...
          <textarea v-model="youtubelink" id="LinkYoutube" class="rounded bg-amber-100  border-black border-2 text-gray-700 w-[500px] h-64 mt-2" placeholder="Cole seu link aqui" type="url"></textarea>
          <button id="buttonYoutube" type="submit" class="block m-auto rounded bg-amber-100 hover:bg-gray-700 py-3 px-4 text-lg text-green-700 disabled:opacity-20 disabled:hover:bg-amber-100 font-bold uppercase mt-10">
            Criar Sala
          </button>
        </div>
        <div class="vl"></div>
        <div class="flex flex-col max-w-lg items-start text-lg mt-6 text-black">
          ...
          <div class="mt-2 flex justify-center items-center w-full">
            <label for="dropzone-file" class="flex flex-col justify-center items-center w-[500px] h-64 bg-amber-100 rounded-lg border-2 border-gray-300 cursor-pointer dark:hover:bg-bray-800 dark:bg-amber-100 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-700">
...
export default {
  ...
  methods:{
    createID(){
      ...
      let text = document.querySelector("#LinkYoutube");
      let button = document.querySelector("#buttonYoutube")
